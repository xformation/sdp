...
var apiUrl = flag.String("apiUrl", "https://sdp.com/api", "api url")
var apiKey = flag.String("apiKey", "", "api key")
var version = ""
var versionRe = regexp.MustCompile(`sdp-(.*)\.(linux|windows)`)
var builds = []build{}
...
func main() {
	...
	postRequest("/sdp/versions", nightly, fmt.Sprintf("Create Release %s", nightly.Version))
	postRequest("/sdp/versions/"+nightly.Version, nightly, fmt.Sprintf("Update Release %s", nightly.Version))

	for _, b := range nightly.Builds {
		postRequest(fmt.Sprintf("/sdp/versions/%s/packages", nightly.Version), b, fmt.Sprintf("Create Build %s %s", b.Os, b.Arch))
		postRequest(fmt.Sprintf("/sdp/versions/%s/packages/%s/%s", nightly.Version, b.Arch, b.Os), b, fmt.Sprintf("Update Build %s %s", b.Os, b.Arch))
	}
}
...
func packageWalker(path string, f os.FileInfo, err error) error {
	...
	builds = append(builds, build{
		Os:     os,
		Arch:   "amd64",
		Url:    "https://s3-us-west-2.amazonaws.com/sdp-releases/master/" + f.Name(),
