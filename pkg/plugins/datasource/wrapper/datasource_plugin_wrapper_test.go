...
import (
	...
	"github.com/xformation/sdp/pkg/tsdb"
...
func TestMappingRowValue(t *testing.T) {
	...
	stringRowValue, _ := dpw.mapRowValue(&datasource.RowValue{Kind: datasource.RowValue_TYPE_STRING, StringValue: "sdp"})
	haveString, ok := stringRowValue.(string)
	if !ok || haveString != "sdp" {
		t.Fatalf("Expected %s, was %s", "sdp", haveString)
	}
