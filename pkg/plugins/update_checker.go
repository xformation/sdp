...
func checkForUpdates() {
	...
	resp, err := httpClient.Get("https://sdp.com/api/plugins/versioncheck?slugIn=" + pluginSlugs + "&grafanaVersion=" + setting.BuildVersion)

	if err != nil {
		log.Trace("Failed to get plugins repo from sdp.net, %v", err.Error())
		return
	}
	...
	if err != nil {
		log.Trace("Update check failed, reading response from sdp.com, %v", err.Error())
		return
	}
	...
	if err != nil {
		log.Trace("Failed to unmarshal plugin repo, reading response from sdp.com, %v", err.Error())
		return
	}
	...
	resp2, err := httpClient.Get("https://raw.githubusercontent.com/sdp/sdp/master/latest.json")
	if err != nil {
