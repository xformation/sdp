...
func runPluginCommand(command func(commandLine CommandLine) error) func(context *cli.Context) {
	return func(context *cli.Context) {
		...
		if err := command(cmd); err != nil {
			...
			logger.Info("\nRestart sdp after installing plugins . <service sdp-server restart>\n\n")
		}
	}
}
...
var adminCommands = []cli.Command{
	{
		...
		Flags: []cli.Flag{
			cli.StringFlag{
				Name:  "homepath",
				Usage: "path to sdp install/home path, defaults to working directory",
			},
			cli.StringFlag{
				Name:  "config",
