	...
	Convey("Email notifier tests", t, func() {
		...
		Convey("Parsing alert notification from settings", func() {
			...
			Convey("from settings", func() {
				...
				So(emailNotifier.Addresses[0], ShouldEqual, "ops@sdp.org")
			})

			Convey("from settings with two emails", func() {
				json := `
				{
					"addresses": "ops@sdp.org;dev@sdp.org"
				}`
				...
				So(emailNotifier.Addresses[0], ShouldEqual, "ops@sdp.org")
				So(emailNotifier.Addresses[1], ShouldEqual, "dev@sdp.org")
			})
		})
	})
}
