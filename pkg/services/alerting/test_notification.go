...
import (
	...
	"github.com/xformation/sdp/pkg/components/null"
...
func createTestEvalContext(cmd *NotificationTestCommand) *EvalContext {
	testRule := &Rule{
		...
		Message:     "Someone is testing the alert notification within sdp.",
		State:       m.AlertStateAlerting,
	}

	ctx := NewEvalContext(context.Background(), testRule)
	if cmd.Settings.Get("uploadImage").MustBool(true) {
		ctx.ImagePublicUrl = "http://sdp.org/assets/img/blog/mixed_styles.png"
	}
	ctx.IsTestRun = true
	ctx.Firing = true
