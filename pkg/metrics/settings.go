...
import (
	...
	"github.com/xformation/sdp/pkg/metrics/graphitebridge"
...
func parseGraphiteSettings(settings *MetricSettings, file *ini.File) (*graphitebridge.Config, error) {
	...
	cfg := &graphitebridge.Config{
		URL:             address,
		Prefix:          graphiteSection.Key("prefix").MustString("prod.sdp.%(instance_name)s"),
	...
	if prefix == "" {
		prefix = "prod.sdp.%(instance_name)s."
	}
