...
func TestLdapAuther(t *testing.T) {
	...
	Convey("When translating ldap user to sdp user", t, func() {
		...
		ldapAutherScenario("Given no existing sdp user", func(sc *scenarioContext) {
			ldapAuther := NewLdapAuthenticator(&LdapServerConf{
				LdapGroups: []*LdapGroupToOrgRole{
	...
	Convey("When syncing ldap groups to sdp org roles", t, func() {
