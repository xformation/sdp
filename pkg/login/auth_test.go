...
func TestAuthenticateUser(t *testing.T) {
	Convey("Authenticate user", t, func() {
		...
		authScenario("When sdp user authenticate with valid credentials", func(sc *authScenarioContext) {
		...
		authScenario("When sdp user authenticate and unexpected error occurs", func(sc *authScenarioContext) {
		...
		authScenario("When a non-existing sdp user authenticate and ldap disabled", func(sc *authScenarioContext) {
			mockLoginAttemptValidation(nil, sc)
		...
		authScenario("When a non-existing sdp user authenticate and invalid ldap credentials", func(sc *authScenarioContext) {
			mockLoginAttemptValidation(nil, sc)
			mockLoginUsingGrafanaDB(m.ErrUserNotFound, sc)
			mockLoginUsingLdap(true, ErrInvalidCredentials, sc)
			mockSaveInvalidLoginAttempt(sc)
		...
		authScenario("When a non-existing sdp user authenticate and valid ldap credentials", func(sc *authScenarioContext) {
			mockLoginAttemptValidation(nil, sc)
			mockLoginUsingGrafanaDB(m.ErrUserNotFound, sc)
			mockLoginUsingLdap(true, nil, sc)
			mockSaveInvalidLoginAttempt(sc)
		...
		authScenario("When a non-existing sdp user authenticate and ldap returns unexpected error", func(sc *authScenarioContext) {
			customErr := errors.New("custom")
			mockLoginAttemptValidation(nil, sc)
			mockLoginUsingGrafanaDB(m.ErrUserNotFound, sc)
		...
		authScenario("When sdp user authenticate with invalid credentials and invalid ldap credentials", func(sc *authScenarioContext) {
			mockLoginAttemptValidation(nil, sc)
