...
var (
	...
	buildNumber           int      = 0
	binaries              []string = []string{"sdp-server", "sdp-cli"}
)
...
func main() {
	...
	for _, cmd := range flag.Args() {
		...
		case "build-cli":
			clean()
			build("sdp-cli", "./pkg/cmd/sdp-cli", []string{})
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		...
		".rpm":    "dist/sdp-latest-1.x86_64.rpm",
		".tar.gz": "dist/sdp-latest.linux-x64.tar.gz",
	}
...
func createDebPackages() {
	createPackage(linuxPackageOptions{
		packageType:            "deb",
		homeDir:                "/usr/share/sdp",
		binPath:                "/usr/sbin",
		configDir:              "/etc/sdp",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/sdp-server",
		initdScriptFilePath:    "/etc/init.d/sdp-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/sdp-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/sdp-server",
		defaultFileSrc: "packaging/deb/default/sdp-server",
		systemdFileSrc: "packaging/deb/systemd/sdp-server.service",
...
func createRpmPackages() {
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/sdp-server",
		initdScriptFilePath:    "/etc/init.d/sdp-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/sdp-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/sdp-server",
		defaultFileSrc: "packaging/rpm/sysconfig/sdp-server",
		systemdFileSrc: "packaging/rpm/systemd/sdp-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "sdp-linux-pack")
...
func setup() {
	...
	runPrint("go", "install", "-v", "./pkg/cmd/sdp-server")
}
...
func clean() {
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/sdp", goos, goarch)))
}
